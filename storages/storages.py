"""
JSON-based storage for the Graph (no external database used).
"""
...
class GraphStorage:
    ...
    def save(graph: Graph, filepath: str | Path) -> None:
        # از to_dict() استفاده می‌کنیم تا هر فیلد دلخواه کاربر
        # (name, username, age, ...) کامل حفظ بشه.
        users_data = [user.to_dict() for user in graph.get_users()]
    ...
    def load(filepath: str | Path) -> Graph:
        ...
        for user_data in data.get("users", []):
            # from_dict() هر ساختاری از فیلدها رو قبول می‌کنه
